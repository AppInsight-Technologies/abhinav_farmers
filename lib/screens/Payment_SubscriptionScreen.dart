...
import 'package:grocbay/screens/paytm_screen.dart';
import 'package:grocbay/widgets/bottom_navigation.dart';
import 'package:grocbay/widgets/custome_stepper.dart';
...
class PaymenSubscriptionScreen extends StatefulWidget {
  ...
  PaymenSubscriptionScreen(Map<String, String> params){
    ...
    this.delcharge = params["deliverycharge"]??"";
...
class _PaymenSubscriptionScreenState extends State<PaymenSubscriptionScreen> with Navigations {
 ...
 var no_of_days;
  ...
  Future<void> _initial() async {
    ...
    if (double.parse(widget.delcharge) <= 0) {
      deliverychargetext = S.current.free;/*"FREE"*/;
    }
    else {
      deliverychargetext =
      Features.iscurrencyformatalign?
      IConstants.numberFormat == "1"?"+ " + double.parse(widget.delcharge).toStringAsFixed(0)  + " " + IConstants.currencyFormat:
      "+ " + double.parse(widget.delcharge).toStringAsFixed(IConstants.decimaldigit) + " " +  IConstants.currencyFormat :
      IConstants.numberFormat == "1"?"+ " + IConstants.currencyFormat + " " + double.parse(widget.delcharge).toStringAsFixed(0):
      "+ " + IConstants.currencyFormat + " " + double.parse(widget.delcharge).toStringAsFixed(IConstants.decimaldigit);
    }
    setState(() {
      if(_isWallet) {
        ...
        if (int.parse(walletbalance) <= 0 ) {
          ...
          if(_isOnline) {
            ...
            remainingAmount =  totalAmount - int.parse(walletbalance) + double.parse(widget.delcharge);
          } else {
          //  _isWallet = false;
           // _ischeckbox = false;
          }
  ...
  Widget build(BuildContext context) {
    ...
    _buildBottomNavigationBar() {
      ...
      return  BottomNaviagation(
        ...
        total: (((subscriptionAmount * quantity) * deliveryNum) + double.parse(widget.delcharge)).toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit),
        onPressed: (){
    ...
    Widget _bodyMobile() {
                   ...
                   // color: ColorCodes.lightBlueColor,
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          Features.iscurrencyformatalign?
                          ((((subscriptionAmount * quantity) * deliveryNum) -((subscriptionAmount * quantity) * deliveryNum)) + double.parse(widget.delcharge) ).toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit) + " " +IConstants.currencyFormat :
                          IConstants.currencyFormat +
                              " " +
                              ((((subscriptionAmount * quantity) * deliveryNum) -((subscriptionAmount * quantity) * deliveryNum)) + double.parse(widget.delcharge) ).toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit),
                          style: TextStyle(
                              fontSize: 13.0,
                              color: ColorCodes.mediumBlueColor,
                              fontWeight: FontWeight.bold),
      ...
      Widget paymentDetails() {
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                width: (Vx.isWeb && !ResponsiveLayout.isSmallScreen(context)) ? (MediaQuery.of(context).size.width * 0.40 -30) : MediaQuery.of(context).size.width,
                decoration: BoxDecoration(
                 color: Colors.white,
                  borderRadius: BorderRadius.all(Radius.circular(5.0)),
                  border: Border.all( color: ColorCodes.greylight)
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                S .of(context).delivery_charge,//"Discount applied:",
                                style: TextStyle(
                                    fontSize: 14.0,
                                    color: ColorCodes.greyColor,
                                    fontWeight: FontWeight.bold),
        ...
        " " + IConstants.currencyFormat :
                              ...
                              Text(
                                "- " +
                                    ((((subscriptionAmount * quantity) * deliveryNum) -((subscriptionAmount * quantity) * deliveryNum)) +double.parse(widget.delcharge) )
                              ...
                              Text(
                                Features.iscurrencyformatalign?
                                (((subscriptionAmount * quantity) * deliveryNum)+ double.parse(widget.delcharge)).toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit) + " " +IConstants.currencyFormat :
                                IConstants.currencyFormat +
                                    " " +
                                    (((subscriptionAmount * quantity) * deliveryNum)+double.parse(widget.delcharge)).toStringAsFixed(IConstants.numberFormat == "1"?0:IConstants.decimaldigit),
                                style: TextStyle(
                                    fontSize: 13.0,
                                    color: ColorCodes.blackColor,
                                    fontWeight: FontWeight.bold),
          ...
          child: (_isWeb) ?
              Column(
                children: [
                  ...
                  SizedBox(width: 20,),
                  paymentDetails(),
                  (Vx.isWeb && !ResponsiveLayout.isSmallScreen(context)) ?   Footer(address: PrefUtils.prefs!.getString("restaurant_address")!) : Container(),
                ],
              )
              :Column(
            crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Future<void> CreateSubscription(int quantity, double subscriptionAmount, int deliveryNum) async {
    ...
    try {
      ...
      resBody["delivery"] = widget.deliveries;
      resBody["start_date"] =widget.startDate;
      resBody["address"] = widget.address;
      ...
      resBody["amount"] = ((subscriptionAmount * quantity) * deliveryNum).toString();
      resBody["branch"] =PrefUtils.prefs!.getString('branch')??"999";
      resBody["slot"] = widget.schedule; //widget.name.toString();
      resBody["payment_type"] = user_data.prepaidModule == "1"? "credits" :(_ischeckbox && _groupValue == 2)?  "wallet": (_ischeckbox && _groupValue == -1)?"wallet":"Paytm";
      resBody["cron_time"] = widget.cronTime;
      resBody["channel"] = channel;
      resBody["var_id"] = widget.varid;
      resBody["type"] = widget.schedule;
      resBody["mrp"] = varmrp;
      resBody["price"] = varprice;
      resBody["quantity"] = widget.itemCount;
      resBody["days"] = text;
      resBody["no_of_days"] = no_of_days;
      resBody["subscriptionType"] = user_data.prepaidModule == "1"? "2" :widget.paymentMode;
      resBody["planId"] = widget.planId;
      resBody["paymentMode"] = paymentMode.toString();
      resBody["walletBalance"] = _ischeckbox?walletAmount.toString():"0";
      resBody["only_wallet"] = (_ischeckbox && _groupValue == -1)? "1":"0";

      if(_checkmembership) {
        resBody["subscriptionDeliveryCharge"] = /*routeArgs['deliveryChargePrime']*//*widget.deliverychargeprime*/widget.delcharge.toString();
      }else{
        resBody["subscriptionDeliveryCharge"] = /* routeArgs['deliveryChargeNormal']*//*widget.deliverychargenormal*/widget.delcharge.toString();
      }
      ...
      final response = await http.post(Api.subscriptionCreate,
          // headers: {"Content-Type": "application/json"},
          body: resBody
        // jsonEncode({
        //     "user_id":PrefUtils.prefs!.getString('apikey'),
        //     "quantity":widget.itemCount,
        //     "delivery":widget.deliveries,
        //     "start_date":widget.startDate,
        //     "address":widget.address,
        //     "address_type":widget.useraddtype,
        //     "address_id":widget.addressid,
        //     "amount":((subscriptionAmount * quantity) * deliveryNum).toString(),
        //     "branch":PrefUtils.prefs!.getString('branch')??"999",
        //     "slot":widget.name,
        //     "payment_type":user_data.prepaidModule == "1"? "credits" :(_ischeckbox && _groupValue == 2)?  "wallet": (_ischeckbox && _groupValue == -1)?"wallet":"Paytm",
        //     "cron_time":widget.cronTime,
        //     "channel":channel,
        //     "var_id": widget.varid,
